...
class PersonalAreaService {
  ...
  Future<PersonalAreaResponseModel> update(
      PersonalAreaRequestModel requestModel) async {
    String loginUrl = "/personal_area/";
    String value = await storage.read(key: "token");
    Map<String, String> headers = {
    ...
    var request = http.MultipartRequest('PUT', Uri.parse(url + loginUrl));
    request.headers.addAll(headers);
    request.fields['request'] = json.encode(requestModel).toString();
    if (requestModel.filePath != null) {
      final file =
  ...
  Future<PersonalAreaRequestModel> getById(String id) async {
    String loginUrl = "/personal_area/" + id;
    String value = await storage.read(key: "token");
    Map<String, String> headers = {
      'content-type': 'application/json',
      'x-access-token': value
